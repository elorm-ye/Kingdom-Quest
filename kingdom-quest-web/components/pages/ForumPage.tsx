...
import { MessageMultiple01Icon, Message01Icon, ArrowUp01Icon, Add01Icon } from '@hugeicons/core-free-icons';
    ...
    <div className="p-6 max-w-4xl mx-auto animate-fade-in">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        ...
        <button className="kq-btn kq-btn-primary self-start md:self-auto">
          <HugeiconsIcon icon={Add01Icon} size={18} /> New Post
        </button>
      </header>
          ...
          {posts.map(post => (
            <div key={post.id} className="kq-card p-0 flex">
              <div className="p-4 flex flex-col items-center justify-start border-r bg-gray-50/50" style={{ borderColor: 'var(--border-subtle)', minWidth: '60px' }}>
                <button className="p-1 hover:text-green-600 transition-colors" style={{ color: 'var(--text-muted)' }}><HugeiconsIcon icon={ArrowUp01Icon} size={20} /></button>
                <span className="font-bold my-1">{post.vote_score}</span>
                <button className="p-1 hover:text-red-600 transition-colors transform rotate-180" style={{ color: 'var(--text-muted)' }}><HugeiconsIcon icon={ArrowUp01Icon} size={20} /></button>
              </div>
              <div className="p-4 flex-1">
                ...
                <div className="flex items-center gap-4 text-xs font-medium" style={{ color: 'var(--text-muted)' }}>
                  <button className="flex items-center gap-1.5 hover:text-gray-900 transition-colors">
                    <HugeiconsIcon icon={Message01Icon} size={16} /> {post.comment_count} Comments
                  </button>
