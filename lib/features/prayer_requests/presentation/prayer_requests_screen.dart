...
class _PrayerRequestsScreenState extends State<PrayerRequestsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final requests = MockDataService.prayerRequests.where((r) =>
        _selectedCategory == null || r.category == _selectedCategory).toList();

    return Scaffold(
      backgroundColor: isDark ? AppColors.umberNight : AppColors.sand,
      appBar: AppBar(
        leading: IconButton(icon: const Icon(Icons.arrow_back_ios_new, size: 20), onPressed: () => context.pop()),
        title: Text('Prayer Requests', style: GoogleFonts.bricolageGrotesque(fontSize: 20, fontWeight: FontWeight.w600)),
      ),
      ...
      body: Column(children: [
        ...
        SizedBox(
          ...
          child: ListView(
            ...
            children: [
              _chip('All', _selectedCategory == null, () => setState(() => _selectedCategory = null), isDark),
              ...PrayerCategory.values.map((c) =>
                  _chip('${c.icon} ${c.label}', _selectedCategory == c, () => setState(() => _selectedCategory = c), isDark)),
            ],
          ),
  ...
  Widget _chip(String label, bool selected, VoidCallback onTap, bool isDark) {
    ...
    final fg = selected ? Colors.white : (isDark ? AppColors.textSecondaryDark : AppColors.umber);
    return Padding(
      ...
      child: GestureDetector(
        ...
        child: Container(
          ...
          child: Center(child: Text(label, style: GoogleFonts.schibstedGrotesk(fontSize: 12, fontWeight: FontWeight.w500, color: fg))),
        ),
      ),
    );
  }
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Container(
            ...
            child: Text(req.status.label, style: GoogleFonts.schibstedGrotesk(fontSize: 10, fontWeight: FontWeight.w600, color: statusColor)),
          ),
          const Spacer(),
          Text('${req.category.icon} ${req.category.label}',
              style: GoogleFonts.schibstedGrotesk(fontSize: 11, color: isDark ? AppColors.textMutedDark : AppColors.muted)),
        ]),
        const SizedBox(height: AppSpacing.md),
        Text(req.title, style: GoogleFonts.bricolageGrotesque(
        ...
        Text(req.description, maxLines: 2, overflow: TextOverflow.ellipsis,
            style: GoogleFonts.schibstedGrotesk(fontSize: 13, height: 1.5,
        ...
        Row(children: [
          ...
          Text(req.isAnonymous ? (req.anonymousDisplayName ?? 'Anonymous') : (req.submitterName ?? 'Member'),
              style: GoogleFonts.schibstedGrotesk(fontSize: 12, color: isDark ? AppColors.textMutedDark : AppColors.muted)),
          const Spacer(),
          Icon(Icons.favorite_outline, size: 14, color: AppColors.terracotta),
          const SizedBox(width: 4),
          Text('${req.prayerCount} praying',
              style: GoogleFonts.schibstedGrotesk(fontSize: 12, color: isDark ? AppColors.textMutedDark : AppColors.muted)),
        ]),
        if (req.responses.isNotEmpty) ...[
          ...
          const SizedBox(height: AppSpacing.sm),
          Row(children: [
            Icon(Icons.reply, size: 14, color: isDark ? AppColors.accentLinkDark : AppColors.terracotta),
            const SizedBox(width: 4),
            Text('Response from ${req.responses.first.adminName}',
                style: GoogleFonts.schibstedGrotesk(fontSize: 12, fontWeight: FontWeight.w500,
                    color: isDark ? AppColors.accentLinkDark : AppColors.terracotta)),
          ]),
          const SizedBox(height: AppSpacing.xs),
          Text(req.responses.first.message, maxLines: 2, overflow: TextOverflow.ellipsis,
              style: GoogleFonts.schibstedGrotesk(fontSize: 12, height: 1.4, fontStyle: FontStyle.italic,
                  color: isDark ? AppColors.textSecondaryDark : AppColors.muted)),
        ],
      ]),
    );
