...
class _SubmitPetitionScreenState extends State<SubmitPetitionScreen> {
  ...
  Future<void> _submit() async {
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Petition submitted successfully.')));
      context.pop();
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('Submit Petition', style: GoogleFonts.bricolageGrotesque(fontSize: 20, fontWeight: FontWeight.w600)),
      ),
      body: SingleChildScrollView(
        ...
        child: Form(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('Share a concern, report an issue, or suggest an improvement.', style: GoogleFonts.schibstedGrotesk(
                fontSize: 14, color: isDark ? AppColors.textMutedDark : AppColors.muted)),
            const SizedBox(height: AppSpacing.xxl),
            _label('Subject', isDark), const SizedBox(height: AppSpacing.sm),
            TextFormField(controller: _subjectCtrl, decoration: InputDecoration(hintText: 'What is this about?', fillColor: card),
                validator: (v) => v == null || v.isEmpty ? 'Enter a subject' : null),
            const SizedBox(height: AppSpacing.xl),
            _label('Description', isDark), const SizedBox(height: AppSpacing.sm),
            TextFormField(controller: _descCtrl, maxLines: 5,
                decoration: InputDecoration(hintText: 'Describe in detail...', fillColor: card, alignLabelWithHint: true),
                validator: (v) => v == null || v.isEmpty ? 'Describe your petition' : null),
            const SizedBox(height: AppSpacing.xxl),
            Container(
              ...
              child: Row(children: [
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text('Submit anonymously', style: GoogleFonts.schibstedGrotesk(fontSize: 14, fontWeight: FontWeight.w500,
                      color: isDark ? AppColors.textPrimaryDark : AppColors.umber)),
                  Text('Your identity will be hidden', style: GoogleFonts.schibstedGrotesk(fontSize: 12,
